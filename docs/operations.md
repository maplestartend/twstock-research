...
# 535 collected：本機含吃 data/stock.db 的 prod-DB 整合測試（70 題標 needs_prod_db）
```

> CI（`.github/workflows/ci.yml`）只跑 `-m "not needs_prod_db"` 的 465 題 DB-independent 子集（`data/stock.db` ~1.2GB 不在 git），外加 `dump_openapi --check` / 前端 `tsc` / `scripts/check_bat.py` / gitleaks；SCA（pip-audit / npm audit）報告型不阻擋。
