...
**🆕 收盤 / 即時 切換**：結果區頂端「收盤 / 即時」按鈕。**收盤**（預設）= signal_history 昨收快照。**即時** = 對自選股抓 TWSE mis 盤中報價、用同一份評分引擎重算短/中/綜合分數並依即時綜合分重排（排行卡 + 主表都即時），每 30 秒刷新（盤後 2 分鐘、tab 隱藏暫停）；吃到即時價的列代號旁亮「即時」徽章、價格欄顯示漲跌%。長期分數結構性、不隨盤中價動。抓不到（休市 / 興櫃 / mis 失敗）自動退回收盤值。走 `/api/watchlist/overview/live`，不寫 signal_history。
...
> 「即時」模式**不會寫入 signal_history**（避免污染回測來源）。雷達 / 自選列表頁現在也各有「收盤 / 即時」切換（見下），但同樣只在 API 回應層重算、不落地。
...
**內建策略**：短線強勢、中期波段、長期價值、回檔布局、三榜俱佳、營收持續成長（≥3 月 YoY>0）、營收高速加速（≥3 月 YoY>20%）、量能動能。
> 2026-06 移除績效後段班 3 個策略（相對強勢、月營收爆發、外資連買）——以 381 個歷史快照、固定 20/60 日 forward return 排名，這三者的 forward-return 中位數與勝率同時墊底（月營收爆發中位數為負且每日命中數最高=雜訊最多）。
...
**🆕 收盤 / 即時 切換**（命中表上方）：開盤後想選股時，切「即時」讓分數跟著盤中價走。**收盤**（預設）= 昨收快照。**即時** = 對**當前這一頁**（≤50 檔）抓 TWSE mis 盤中報價、用同一份引擎重算短/中/綜合分數，並**在當前頁內依即時分數重排**（成本有界、不跨頁拉資料），每 30 秒刷新（盤後 2 分鐘、tab 隱藏暫停）。吃到即時價的列亮「即時」徽章、收盤欄顯示即時價 + 漲跌%。長期分數結構性、不隨盤中價動。走 `/api/radar/hits/live`，不寫 signal_history。
